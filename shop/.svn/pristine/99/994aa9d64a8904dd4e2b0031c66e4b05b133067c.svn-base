...
<script type="text/javascript">
 ...
 // 페이징 설정 
	var dateFoodPageSize = 10;    	// 화면에 뿌릴 데이터 수 
	var dateFoodPageBlock = 5;		// 블럭으로 잡히는 페이징처리 수
...
$(document).ready(function() {
	...
	function dateFoodSaleList(currentPage) {
		...
		callAjax("/stats/dateFoodSaleList.do", "post", "text", true, param, resultCallback);
	}
	
	function fBoardResult(data, currentPage) {
		
		
		$('#dateFoodSaleList').empty();
		...
		var $dateFoodSaleList = $data.find("#dateFoodSaleList");
		$("#dateFoodSaleList").append($dateFoodSaleList.children());
		
		
		var $totalCnt = $data.find("#totalCnt");
		var totalCnt = $totalCnt.text();
		
		var paginationHtml = getPaginationHtml(currentPage, totalCnt, dateFoodPageSize, dateFoodPageBlock, 'dateFoodSaleList'); 
					...
					<div class="content">
						...
						<p class="Location">
							...
							<a href="#" class="btn_nav">통계</a> 
								<span class="btn_nav bold">일자별 음식 판매 목록</span> 
								<a href="#" class="btn_set refresh">새로고침</a>
						</p>

						<p class="conTitle">
							<span>일자별 음식 판매 목록 </span> <span class="fr"></span>
						</p>
