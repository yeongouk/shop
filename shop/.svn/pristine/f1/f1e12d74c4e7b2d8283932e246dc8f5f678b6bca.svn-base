...
<script type="text/javascript">
 ...
 // 페이징 설정 
	var ctmPageSize = 10;    	// 화면에 뿌릴 데이터 수 
...
$(document).ready(function() {
	...
	function ctmBuyRankingList(currentPage) {
		...
		callAjax("/stats/ctmBuyRankingList.do", "post", "text", true, param, resultCallback);
	}
	
	function fBoardResult(data, currentPage) {
		
		
		$('#ctmBuyRankingList').empty();
		...
		var $ctmBuyRankingList = $data.find("#ctmBuyRankingList");
		$("#ctmBuyRankingList").append($ctmBuyRankingList.children());
		
		
		var $totalCnt = $data.find("#totalCnt");
		var totalCnt = $totalCnt.text();
		
		var paginationHtml = getPaginationHtml(currentPage, totalCnt, ctmPageSize, ctmPageBlock, 'ctmBuyRankingList'); 
					...
					<div class="content">
						...
						<p class="Location">
							...
							<a href="#" class="btn_nav">통계</a> 
								<span class="btn_nav bold">구입별 고객 순위</span> 
								<a href="#" class="btn_set refresh">새로고침</a>
						</p>

						<p class="conTitle">
							<span>구입별 고객 순위 </span> <span class="fr"></span>
						</p>
