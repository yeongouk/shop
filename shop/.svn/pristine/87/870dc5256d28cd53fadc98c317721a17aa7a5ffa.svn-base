...
<script type="text/javascript">
	...
	function fRegisterButtonClickEvent(){
		$('a[name=btn]').click(function(e){
			...
			switch(btnId){
			case 'btnSaveDtlCod' : fSaveAdvice(); // save 안에 저장,수정
				//alert("저장버튼 클릭!!!이벤트!!");
				break;
			case 'btnDeleteDtlCod' : fDeleteAdvice();	// 만들자 
				//alert("삭제버튼 클릭!!!이벤트!!");		
				break;
			case 'btnCloseDtlCod' : gfCloseModal();  // 모달닫기 
			fListComnGrpCod(); // 첫페이지 다시 로딩 
				break;
			case 'btnUpdateDtlCod' : fUpdateAdvice();  // 수정하기
				break;
	...
	/* 저장 ,수정, 삭제 콜백 함수 처리  */   
	 function fSaveAdviceResult(data){
	...
	function fListComnGrpCod(currentPage) {
		...
		callAjax("/supportA/listAdviceMgt.do", "post", "text", true, param, resultCallback);
	}
	...
	function flistGrpCodResult(data, currentPage) {
		...
		// 신규 목록 생성
		var $listAdviceMgt = $data.find("#listAdviceMgt");		
		$("#listAdviceMgt").append($listAdviceMgt.children());
		
		// 총 개수 추출
		var $totalCntAdviceMgt = $data.find("#totalCntAdviceMgt");
		var totalCntAdviceMgt = $totalCntAdviceMgt.text(); 
		
		// 페이지 네비게이션 생성
		var paginationHtml = getPaginationHtml(currentPage, totalCntAdviceMgt, pageSizeComnGrpCod, pageBlockSizeComnGrpCod, 'fListComnGrpCod');
	...
	function fSaveGrpCod() {
		...
		callAjax("/supportA/saveAdviceMgt.do", "post", "json", true, $("#myForm").serialize(), resultCallback);
	}
	...
	function fListComnDtlCod(currentPage, no, loginID) {
		...
		callAjax("/supportA/listAdviceMgtDtl.do", "post", "text", true, param, resultCallback);
	}
	...
	function flistDtlCodResult(data, currentPage) {
		...
		// 기존 목록 삭제
		$('#listAdviceMgtDtl').empty(); 
		
		var $data = $( $(data).html() );

		// 신규 목록 생성
		var $listAdviceMgtDtl = $data.find("#listAdviceMgtDtl");		
		$("#listAdviceMgtDtl").append($listAdviceMgtDtl.children());
		
		// 총 개수 추출
		var $totalCntAdviceMgtDtlCod = $data.find("#totalCntAdviceMgtDtlCod");
		var totalCntAdviceMgtDtlCod = $totalCntAdviceMgtDtlCod.text(); 
		
		// 페이지 네비게이션 생성
		var no = $("#tmpGrpCod").val();
		var loginID = $("#tmpGrpCodNm").val();
		var paginationHtml = getPaginationHtml(currentPage, totalCntAdviceMgtDtlCod, 
	...
	function fListComnDtlCod2(currentPage, loginID, reg_date) {
		...
		callAjax("/supportA/listAdviceMgtDtl2.do", "post", "text", true, param, resultCallback);
	}
	...
	function flistDtlCodResult2(data, currentPage) {
		...
		// 기존 목록 삭제
		$('#listAdviceMgtDtl2').empty(); 
		
		var $data = $( $(data).html() );

		// 신규 목록 생성
		var $listAdviceMgtDtl2 = $data.find("#listAdviceMgtDtl2");		
		$("#listAdviceMgtDtl2").append($listAdviceMgtDtl2.children());
		
		// 총 개수 추출
		var $totalCntAdviceMgtDtlCod2 = $data.find("#totalCntAdviceMgtDtlCod2");
		var totalCntAdviceMgtDtlCod2 = $totalCntAdviceMgtDtlCod2.text(); 
		
		// 페이지 네비게이션 생성
		var loginID = $("#tmpGrpCod2").val();
		var reg_date = $("#tmpGrpCodNm2").val();
		var paginationHtml = getPaginationHtml(currentPage, totalCntAdviceMgtDtlCod2, 
	...
	/** 상담정보 단건 조회 */
	function fSelectDtlCod(loginID) {
		...
		callAjax("/supportA/selectAdviceMgtDtl.do", "post", "json", true, param, resultCallback);
	}
	...
	function fSelectDtlCodResult(data) {
		...
		if (data.result == "SUCCESS") {
			...
			//폼 데이터 설정
			fInitFormDtlCod(data.adviceMgtDtlModel);
			// 모달 팝업
			gfModalPop("#layer2");			
	 ...
	 function fSaveAdvice(){
		...
		// if(!(fValidatePopup())){ return; }
		 ...
		 var resultCallback3 = function(data){
			 fSaveAdviceResult(data);
		 };
		 
		 $("#action").val("I");  // insert
		 
		 callAjax("/supportA/AdviceMgtSave.do", "post", "json", true, $("#myForm").serialize(), resultCallback3);
	 	// $("#myNotice").serialize() => 직렬화해서 name 값들을 그냥 넘김.
	 }
	
	
	 /* 공지사항 등록(수정) */
	 function fUpdateAdvice(){
					...
					<div class="content">
						...
						<p class="Location">
							<a href="#" class="btn_set home">메인으로</a> <a href="#"
								class="btn_nav">관리자페이지</a> <span class="btn_nav bold">수강상담관리</span> <a href="#" class="btn_set refresh">새로고침</a>
						</p>
						<p class="conTitle">
							<span>수강상담 관리 </span> 
				            </p>
						...
						<p class="conTitle mt50">
							<span>상담정보</span> <span class="fr"> <a
				...
				<table class="row">
					...
					<tbody>
						<tr>
							...
							<td><input type="text" class="inputTxt p30" id="no" name="no" /></td>
							...
							<td><input type="text" class="inputTxt p500" id="counsel" name="counsel" /></td>
							<th scope="row">상담가<span class="font_red">*</span></th>
							<td><input type="text" class="inputTxt p100" id="regId" name="regId" /></td>
							</tr>
							 <tr>
