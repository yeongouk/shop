...
<script type="text/javascript">
 ...
 // 페이징 설정 
	var foodPageSize = 10;    	// 화면에 뿌릴 데이터 수 
	var foodPageBlock = 5;		// 블럭으로 잡히는 페이징처리 수
...
$(document).ready(function() {
	...
	function foodRankingList(currentPage) {
		...
		callAjax("/stats/foodRankingList.do", "post", "text", true, param, resultCallback);
	}
	
	function fBoardResult(data, currentPage) {
		
		
		$('#foodRankingList').empty();
		...
		var $foodRankingList = $data.find("#foodRankingList");
		$("#foodRankingList").append($foodRankingList.children());
		
		
		var $totalCntFoodRK = $data.find("#totalCntFoodRK");
		var totalCntFoodRK = $totalCntFoodRK.text();
		
		var paginationHtml = getPaginationHtml(currentPage, totalCntFoodRK, foodPageSize, foodPageBlock, 'foodRankingList'); 
					...
					<div class="content">
						...
						<p class="Location">
							...
							<a href="#" class="btn_nav">통계</a> 
								<span class="btn_nav bold">음식 판매 순위</span> 
								<a href="#" class="btn_set refresh">새로고침</a>
						</p>

						<p class="conTitle">
							<span>음식 판매 순위 </span> <span class="fr"></span>
						</p>
                  ...
                  <!--검색창  -->
					<table width="100%" cellpadding="5" cellspacing="0" border="1"
                        ...
                        <tr style="border: 0px; border-color: blue">
                           ...
                           <td width="50" height="25" style="font-size: 100%">날짜</td>
