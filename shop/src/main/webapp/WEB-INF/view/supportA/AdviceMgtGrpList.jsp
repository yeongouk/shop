...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
					<div class="divEmpty">
						<div class="hiddenData">
							<span id="totalCntAdviceMgt">${totalCntAdviceMgt}</span>
						</div>
						<table class="col">
							<thead>
								<tr>
							...
							<tbody id="listAdviceMgt">
							<c:if test="${totalCntAdviceMgt eq 0 }">
								<tr>
									<td colspan="9">데이터가 존재하지 않습니다.</td>
								</tr>
							</c:if>
							<c:set var="nRow" value="${pageSize*(currentPageComnGrpCod-1)}" />
							<c:forEach items="${listAdviceMgtModel}" var="list">
								<tr>
									<td>${totalCntAdviceMgt - nRow}</td>
									<td>${list.no}</td>
